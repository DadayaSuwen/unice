  ...
  for (const p of productDefs) {
    ...
    await payload.create({
      ...
      data: {
        ...
        safety_info: (p.safety_info || []).map((s: any) => ({
          title: s.title,
          content: s.content,
        })),
      },
  ...
  const newsDefs = [
    {
      ...
      excerpt:
        "凭借在新材料研发领域的突出贡献，江西联合化工荣获中国化工协会颁发的年度创新奖。",
...
- 环保性能突出，符合欧盟REACH法规`,
    ...
    },
    {
      ...
      excerpt:
        "江西联合化工与德国巴斯夫公司签署战略合作协议，双方将在技术研发、市场拓展等领域开展深度合作。",
...
此次合作将聚焦高性能环保树脂的联合研发，双方将共享研发资源，共同推进新材料在汽车、电子等高端领域的应用。`,
  ...
  for (const c of careerDefs) {
    const existing = await payload.find({
      collection: "careers",
      where: { position: { equals: c.position } },
      limit: 1,
    });
    if (existing.docs.length === 0) {
      await payload.create({
        ...
        data: {
          ...
          responsibilities: c.responsibilities.map((text: string) => ({
            text,
          })),
          career_benefits: [],
        },
  ...
  const bannerDefs = [
    {
      title: "专业化工原料制造商",
      subtitle: "20年行业经验，为全球客户提供卓越化工解决方案",
      ...
      features: {
        ...
        features: [
          ...
          },
          {
            icon: "bolt",
            title: "创新技术",
            description: "拥有50+项专利技术，持续投入研发，推动行业技术进步",
          },
          {
            icon: "shield",
            title: "安全环保",
            description: "严格遵循EHS标准，绿色生产工艺，致力于可持续发展",
          },
          {
            icon: "globe",
            title: "全球供应",
            description: "覆盖50+国家和地区的供应链网络，确保产品及时交付",
          },
          {
            icon: "team",
            title: "专业团队",
            description: "200+专业技术人员，提供从咨询到售后的一站式服务",
          },
          {
            icon: "spark",
            title: "定制方案",
            description: "深入理解客户需求，提供个性化的产品解决方案",
          },
        ],
      },
      factory: {
    ...
    data: {
      ...
      rdCards: [
        ...
        },
        {
          icon: "team",
          title: "专家团队",
          description: "汇聚国内外化工领域资深专家，为产品研发提供强大支持。",
        },
      ],
    },
  });
